...
class M2DETRDetectionModel(RTDETRDetectionModel):
    """M2DETR model with RT-DETR detection loss and an image-level classification loss."""

    def __init__(self, cfg="m2detr.yaml", ch=3, nc=None, verbose=True):
        """Initialize M2DETR."""
        super().__init__(cfg=cfg, ch=ch, nc=nc, verbose=verbose)
        head = self.model[-1]
        self.nc = self.yaml["nc"]
        self.cls_nc = getattr(head, "cls_nc", self.yaml.get("cls_nc", 2))
        self.cls_names = {i: f"{i}" for i in range(self.cls_nc)}

    def init_criterion(self):
        """Initialize the M2DETR multitask criterion."""
        from ultralytics.models.m2detr.loss import M2DETRLoss

        head = self.model[-1]
        return M2DETRLoss(
            nc=self.nc,
            cls_nc=self.cls_nc,
            cls_loss_gain=getattr(head, "cls_loss_gain", 1.0),
            cls_label_smoothing=self.yaml.get("cls_label_smoothing", 0.0),
            use_vfl=True,
        )

    @staticmethod
    def _get_image_cls(batch: dict, img: torch.Tensor) -> torch.Tensor | None:
        """Fetch image-level class labels from common batch keys."""
        for key in ("im_cls", "image_cls", "cls_label", "image_label", "labels", "classification"):
            ...
            if isinstance(labels, torch.Tensor):
                ...
                labels = torch.stack([x.reshape(()) if x.numel() == 1 else x.reshape(-1)[0] for x in labels]).to(
                    img.device
                )
            else:
                labels = torch.as_tensor(labels, device=img.device)
            labels = labels.view(-1)
            if labels.numel() == img.shape[0]:
                return labels.long()
        return None

    def loss(self, batch, preds=None):
        """Compute M2DETR detection and image classification losses."""
        ...
        pred_pack = preds if self.training else preds[1]
        cls_logits = None
        if isinstance(pred_pack, dict):
            cls_logits = pred_pack.get("cls_logits")
            pred_pack = pred_pack.get("det")

        if len(pred_pack) == 6:
            dec_bboxes, dec_scores, enc_bboxes, enc_scores, dn_meta, cls_logits = pred_pack
        else:
            dec_bboxes, dec_scores, enc_bboxes, enc_scores, dn_meta = pred_pack
        ...
        dec_scores = torch.cat([enc_scores.unsqueeze(0), dec_scores])
        image_cls = self._get_image_cls(batch, img)
...
def parse_model(d, ch, verbose=True):
    ...
    cls_nc = d.get("cls_nc", d.get("image_nc", nc))
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
        ...
        elif m in frozenset({RTDETRDecoder, M2RTDETRDecoder}):  # channels arg must be passed in index 1
            ...
            c2 = ch[f[-1]]
        elif m in frozenset({TorchVision, M2TimmBackbone, Index}):
            c2 = args[0]
            c1 = ch[f]
            args = [*args[1:]]
        else:
...
def guess_model_scale(model_path):
    ...
    if stem == "m2detr":
        return "n"
    if match := re.search(r"(?:m2detr|rtdetr)-([nslmx])$", stem):
        return match.group(1)
    if "m2detr" in Path(model_path).parts:
        return "n"
    try:
        return re.search(r"yolo(e-)?[v]?\d+([nslmx])", stem).group(2)
    ...
    if isinstance(model, torch.nn.Module):  # PyTorch model
        ...
        for m in model.modules():
            if isinstance(m, SemanticSegment):
                ...
                return "detect"
            elif isinstance(m, M2RTDETRDecoder):
                return "detect"
